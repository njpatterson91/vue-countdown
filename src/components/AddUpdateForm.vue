    ...
    <form @keyup.enter.prevent="validate()">
      ...
      <div>
        ...
        <select id="background" v-model="event.background">
          ...
          <option value="#EB9AoF">Orange</option>
        </select>
      </div>
      <div>
        <button v-if="event.id" @click.prevent="validate('update')">
          update
        </button>
        <button v-else @click.prevent="validate('add')">
          add
        </button>
      </div>
    </form>
...
export default {
  ...
  methods: {
    ...
    validate(type) {
      ...
      if (!this.event.details) this.errors.push("Details are required");
      if (!this.event.date) this.errors.push("Date is required");
